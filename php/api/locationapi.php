...
// Fetch all skills and their emoji
$skill_emoji = [];
$skill_result = $conn->query("SELECT name, emoji FROM skills");
while ($row = $skill_result->fetch_assoc()) {
    $skill_emoji[$row['name']] = $row['emoji'];
}

// Fetch all services and their emoji
$service_emoji = [];
$service_result = $conn->query("SELECT name, emoji FROM services");
while ($row = $service_result->fetch_assoc()) {
    $service_emoji[$row['name']] = $row['emoji'];
}
...
?>
    <div style="padding:18px 0 0 18px;">
        <a href="../pages/services.php" style="display:inline-flex;align-items:center;gap:6px;background:#1B4D43;color:#fff;padding:7px 18px;border-radius:7px;text-decoration:none;font-weight:600;font-size:1.05em;box-shadow:0 2px 8px rgba(27,77,67,0.10);transition:background 0.18s;">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24"><path fill="currentColor" d="M15.41 7.41 14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>
            Back to Services
        </a>
    </div>
    <div id="container">
    ...
    <script>
    const skillEmoji = <?php echo json_encode($skill_emoji); ?>;
    const serviceEmoji = <?php echo json_encode($service_emoji); ?>;
    ...
    function renderTags(worker) {
        ...
        if (worker.skills) {
            ...
            skillsArr.slice(0,2).forEach(skill => {
                ...
                tags.push(`<span class="worker-tag" title="${skill}">${emoji} ${skill.length > 16 ? skill.slice(0,14)+'…' : skill}</span>`);
            });
        }
        if (worker.services) {
            ...
            if (servicesArr.length > 0) {
                ...
                tags.push(`<span class="worker-tag service" title="${servicesArr[0]}">${emoji} ${servicesArr[0].length > 16 ? servicesArr[0].slice(0,14)+'…' : servicesArr[0]}</span>`);
            }
        }
    ...
    const workerIcon = L.icon({
        iconUrl: '../php/assets/logo_ss.png',
        iconSize: [40, 40],
        iconAnchor: [20, 40],
        popupAnchor: [0, -40]
    });
    ...
    function renderWorkerList(filteredWorkers) {
        ...
        filteredWorkers.forEach((worker, idx) => {
            ...
            div.innerHTML = `
                ...
                <button class="view-profile-icon" onclick="event.stopPropagation(); showProfileModal(${idx});" title="View Profile">
                    <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
                        <path d="M12 5c-7.633 0-10 6.5-10 7s2.367 7 10 7 10-6.5 10-7-2.367-7-10-7zm0 12c-5.053 0-7.507-3.6-8.445-5 .938-1.4 3.392-5 8.445-5 5.053 0 7.507 3.6 8.445 5-.938 1.4-3.392 5-8.445 5zm0-8a3 3 0 100 6 3 3 0 000-6zm0 4a1 1 0 110-2 1 1 0 010 2z"/>
                    </svg>
                </button>
            `;
            div.onclick = () => {
                    ...
                    const popupHtml = `
                        <div class="popup-profile" style="font-family:Inter,Arial,sans-serif;min-width:220px;">
                            ...
                            <div style="margin-top:12px;display:flex;justify-content:space-between;align-items:center;">
                                <button onclick="showProfileModal(${i});" style="background:#1B4D43;border:2px solid #1B4D43;border-radius:7px;padding:7px 18px;font-size:1em;font-weight:700;color:#fff;cursor:pointer;transition:background 0.18s,border 0.18s;">View Full Profile</button>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['logout'])) {
    session_unset();
    session_destroy();
    header("Location: /Skill_Sphere/php/home_page.php");
    exit;
}
?>